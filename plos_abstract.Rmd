...
# Describe the data, then clean it
...
However, a fair number of the structured abstracts only partially parsed before dying - we can look with the length of the abstract in characters.
...
Need to put this at the end, but here is the giant chunk now?
...
# That analysis went over like a wet fart
...
# todo text2vec glove/ topic modeling etc...
...
reader = function(x,...) {
  # read
  res= read_file(x)
  fname=strsplit(x,"/")[[1]][4]
  id=strsplit(fname,"_")[[1]][1]
  names(res)=id
  res
}
# create iterator over files
it_files  = ifiles(dff$abstract_file, reader = reader)
# create iterator over tokens from files iterator
it_tokens = itoken(it_files, preprocess_function = tolower, 
                    tokenizer = tokenize_words, progessbar = FALSE)

vocab = create_vocabulary(it_tokens)
pruned_vocab=prune_vocabulary(vocab,term_count_min=10,doc_proportion_max=.5,doc_proportion_min=0.001)
...
pruned_vocab=prune_vocabulary(vocab,term_count_min=10,doc_proportion_max=.5,doc_proportion_min=0.001)
vectorizer = vocab_vectorizer(pruned_vocab)
dtm = create_dtm(it_tokens, vectorizer, type = "lda_c")

lda_model = 
  LDA$new(n_topics = 10, vocabulary = vocab, 
